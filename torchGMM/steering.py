...
def steered_reverse_edm_sampling(
    ...
    `x_next = x_noisy
              + step_scale * (sigma_next**2 - sigma_hat**2) / 2
                           * drift(x_noisy, sigma_hat, ...)`.

    The first step cannot reheat because no larger grid level exists.
    Reheating and deterministic transport are separate operators; this is not
    an Euler-Maruyama step. Every drift evaluation remains on the supplied
    Karras grid.
    ...
    Args:
        drift: `(x_noisy, sigma_hat, sigma_curr, sigma_next) ->
            [N, *rest, D]`; complete deterministic drift with respect to
            `sigma**2 / 2`, including any caller-provided guidance.
    ...
    for step, (sigma_curr, sigma_next) in enumerate(zip(sigma[:-1], sigma[1:])):
        ...
        drift_ = drift(x, sigma_hat, sigma_curr, sigma_next)
        x = x + step_scale * (-1)*(sigma_hat**2 - sigma_next**2) / 2 * drift_
